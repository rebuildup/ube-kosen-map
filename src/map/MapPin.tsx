 ...
 * Icons replaced with a white dot (lucide-react removed).
 */

import type React from "react";
import type { Coordinate, LabelPosition } from "./types";

interface MapPinProps {
  id: string;
  position: { x: number; y: number };
  svgCoordinate: Coordinate;
  type: "event" | "exhibit" | "room" | "stall" | "toilet" | "trash";
  label?: string;
 ...
 * Icon replaced with a simple white dot at pin center.
 */
export const MapPin: React.FC<MapPinProps> = ({
  ...
  // Google Maps standard pin dimensions
  const PIN_WIDTH = 24;
  const PIN_HEIGHT = 32;

  // Drop shadow matching the source spec
  const SHADOW =
  ...
  return (
    <button
      ...
      style={{
        ...
        position: "absolute",
        top: `${position.y}px`,
        // Bottom-center of pin anchors to position coordinate
    ...
    >
      ...
      >
        ...
        <circle
          ...
          strokeWidth="2"
        />
        {/* White center dot instead of lucide-react icon */}
        <circle cx={PIN_WIDTH / 2} cy={PIN_WIDTH / 2} r="3" fill="white" />
      </svg>
...
/**
 * Google Maps style Cluster Pin with conic-gradient pie chart background.
 */
...
export const ClusterPin: React.FC<ClusterPinProps> = ({
  ...
  return (
    <button
      ...
      onTouchEnd={onTouchEnd}
    >
      {/* Pin SVG - same teardrop shape, larger for cluster */}
      <svg
        width={PIN_WIDTH}
        height={PIN_HEIGHT}
        viewBox={`0 0 ${PIN_WIDTH} ${PIN_HEIGHT}`}
      ...
      {/* Conic-gradient pie chart circle */}
      <div
        style={{
...
/**
 * Highlight Pin with pulse animation (@keyframes ping defined in theme.css).
 */
...
export const HighlightPin: React.FC<HighlightPinProps> = ({ position }) => {
  ...
  return (
    ...
    >
      {/* Pulse ring - uses @keyframes ping from theme.css */}
      <div
        style={{
